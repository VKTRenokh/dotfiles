...
local function _5_()
  keymap({"n"}, "<space>gh", history_toggle, "Toggle git history")
  keymap({"x"}, "<space>gh", ":DiffviewFileHistory<cr>", "Toggle git history")
  return keymap({"n"}, "<space>gv", ":DiffviewOpen", "DiffviewOpen")
end
local function _6_()
  ...
  local diffview_unmap = {["]x"] = false, ["<leader>e"] = false, dx = false, ["<leader>b"] = false, ["<leader>co"] = false, ["<leader>ct"] = false, ["<leader>cb"] = false, ["<esc>"] = false, q = false, ["<leader>ca"] = false, ["[x"] = false}
...
return tx("sindrets/diffview.nvim", {lazy = true, cmd = {"DiffviewOpen", "DiffviewFileHistory"}, init = _5_, config = _6_})
